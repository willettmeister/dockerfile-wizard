...
#     echo "deb http://ftp.debian.org/debian main" >> /etc/apt/sources.list && \\
...
;  else \\
		apt-get update && DEBIAN_FRONTEND=noninteractive apt-get -y install xvfb phantomjs \\
; fi
EOF
echo "ENV DISPLAY :99"
...
RUN apt-get -y install libgconf-2-4 \
  && curl --silent --show-error --location --fail --retry 3 --output /tmp/chromedriver_linux64.zip \"https://chromedriver.storage.googleapis.com/75.0.3770.90/chromedriver_linux64.zip\" \
  && cd /tmp \
...
echo "# Install program to configure locales
